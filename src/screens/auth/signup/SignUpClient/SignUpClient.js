...
const SignUpClient = ({ navigation }) => {
  ...
  const handleSignUp = async () => {
    ...
    try {
      const response = await axios.post('http://192.168.1.4:5001/registerClient', {
        name,
        email,
        phone,
        password,
      ...
      navigation.navigate('SignInClient');
    } catch (error) {
  ...
  return (
    <Container>
      ...
      <View style={{ display: 'flex', flexDirection: 'row' }}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate('SignInClient')}>
          <Text
            style={{
